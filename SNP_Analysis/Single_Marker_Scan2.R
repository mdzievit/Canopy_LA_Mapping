...
##Read phenotype data
pheno <- read_tsv("Filtered_SNPs/Phenotype_Data/LA_BLUPs.txt") %>%
  gather(Trait,BLUP,-Gen)

#Read coded SNP data in before it sliding window
# data <- read_tsv("Filtered_SNPs/B73_Population/B73_Progeny_Filter1.map",
#                  na = "-") %>%
...
#          BP = as.numeric(BP),
#          SNP2 = row_number())
# write_tsv(path = "Filtered_SNPs/B73_Population/B73_single_marker_scan_snps.txt",
#           x = b73_snps)
# 
# model_out <-  data %>%
#   group_by(Trait,SNP) %>%
...
#   arrange(p.value)
# 
# write_tsv(path = "Filtered_SNPs/B73_Population/B73_single_marker_scan_pvalues_BLUPs.txt",
#           x = b73_pvalues)
# 
# rm(data,model_out)
# gc()
...
b73_pvalues <- read_tsv(file = "Filtered_SNPs/B73_Population/B73_single_marker_scan_pvalues_BLUPs.txt")
b73_snps <- read_tsv(file = "Filtered_SNPs/B73_Population/B73_single_marker_scan_snps.txt",
...
ggsave(filename = "Filtered_SNPs/B73_Population/Combined_ComparisonFigure_Supplemental_color_BLUPs.png",
...
##Read phenotype data
pheno <- read_tsv("Filtered_SNPs/Phenotype_Data/LA_BLUPs.txt") %>%
  gather(Trait,BLUE,-Gen)

##Read coded SNP data in before it sliding window
data <- read_tsv("Filtered_SNPs/Mo17_Population/Mo17_Progeny_Filter1.map",
                 na = "-") %>%
  rename(SNP = locus_name) %>%
  gather(Gen,Call,-SNP) %>%
  mutate(Gen = str_replace_all(Gen,c("Proj1." = "",
...
mo17_snps <- data %>%
  ...
  mutate(CHR = as.numeric(str_replace(CHR,"chr","")),
         BP = as.numeric(BP),
         SNP2 = row_number())
write_tsv(path = "Filtered_SNPs/Mo17_Population/Mo17_single_marker_scan_snps.txt",
          x = mo17_snps)
...
write_tsv(path = "Filtered_SNPs/Mo17_Population/Mo17_single_marker_scan_pvalues_BLUPs.txt",
          x = mo17_pvalues)
...
mo17_pvalues <- read_tsv(file = "Filtered_SNPs/Mo17_Population/Mo17_single_marker_scan_pvalues_BLUPs.txt")
mo17_snps <- read_tsv(file = "Filtered_SNPs/Mo17_Population/Mo17_single_marker_scan_snps.txt",
                      col_types = cols(SNP = "c",
                                       CHR = "i",
                                       BP = "d",
                                       SNP2 = "i"))
...
ggsave(filename = "Filtered_SNPs/Mo17_Population/Combined_ComparisonFigure_Supplemental_color_BLUPs.png",
...
ggsave(filename = "Filtered_SNPs/Single_Marker_Both_Pops_BLUPs.png",
       plot = comb,
       dpi = 600)
...
manhattan_plotter(data = mo17_pvalues,
                               cur_Trait = "Ear_Shank",
                               midpoints = mo17_midpoints_man,
                               renumber = mo17_renumber_man)
